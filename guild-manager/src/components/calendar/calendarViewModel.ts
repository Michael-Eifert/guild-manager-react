...
import type { Mission, MissionId } from "../../types/missionTypes";

type RoleCounts = { Tank: number; Healer: number; DPS: number };
type CalendarEventReference = { missionId?: MissionId; missionIds?: MissionId[] };
type RaidMissionOption = {
  key: string;
  label: string;
  missionId?: MissionId;
  missions: Mission[];
...
export const getEventStatusClass = (status: unknown) => {
...
export const isTerminalEventStatus = (status: unknown) =>
  status === CALENDAR_STATUS.COMPLETED || status === CALENDAR_STATUS.CANCELLED;
export const getRaidResetShortLabel = (mission: Mission) => {
  ...
  const missionName = mission.name || "Raid";
  return labels[missionName] || `${missionName} Reset`;
};
export const sortRaidWingsByProgression = (left: Mission, right: Mission) => {
...
export const getRaidMissionOptions = (raidMissions: readonly Mission[]) => {
  const options: RaidMissionOption[] = [];
  const groupedSets = new Map<string, RaidMissionOption>();
  (Array.isArray(raidMissions) ? raidMissions : []).forEach((mission) => {
    if (mission?.dungeonSetId && mission?.dungeonSetName) {
      ...
      if (!groupedSets.has(key)) {
        const option: RaidMissionOption = { key, label: mission.dungeonSetName, missions: [] };
...
export const getEventMissionIds = (event: CalendarEventReference) =>
  Array.isArray(event?.missionIds) && event.missionIds.length > 0
    ? event.missionIds
    : event?.missionId ? [event.missionId] : [];
export const isRaidResetDay = (mission: Mission, dayIndex: number) => {
