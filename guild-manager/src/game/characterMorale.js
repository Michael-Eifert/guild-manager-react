...
export const clampMorale = (value) => {
...
export const getCharacterMorale = (character) =>
  clampMorale(character?.morale ?? DEFAULT_MORALE);

export const getMoraleBand = (moraleValue) => {
...
export const getMoraleLabel = (moraleValue) => {
...
export const getMoraleSuccessModifier = (character) => {
...
export const getPartyMoraleSuccessBonus = (members) => {
...
export const applyMoraleDelta = (character, delta) => ({
  ...character,
  morale: clampMorale(getCharacterMorale(character) + (Number(delta) || 0)),
});

export const isCharacterInZoneLevelRange = (character, zone) => {
...
export const isCharacterInMissionLevelRange = (character, mission) => {
  const level = Math.max(1, Number(character?.level) || 1);
